.. duo documentation master file, created by
   sphinx-quickstart on Thu Jun 10 14:14:54 2021.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Duo Developers Manual
===============================

.. note::

   Duo, a Python API (GNU General Public License v3.0), is an Effective Atomic Number (EAN) calculation engine, and part of the 3D Quantitative Imaging (3DQI) software developed by `3DQI laboratory at Radiology department <https://3dqi.github.io/3dqi_website/index.html?pageName=a-home>`_, Massachusetts General Hospital (MGH).

.. note::

   3DQI is a research software for medical image analysis. The binaries are free to use, and can be requested by sending an email to ``Cai.Wenli@mgh.harvard.edu``.

.. toctree::
   :maxdepth: 2
   :caption: Contents:

   installation
   api

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
